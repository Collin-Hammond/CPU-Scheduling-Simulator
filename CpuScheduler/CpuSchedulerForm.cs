...
4. Compare algorithm behaviors and outcomes";
        ...
        /// Always runs the arrived process with the shortest remaining burst time
        /// </summary>
        private List<SchedulingResult> RunSRTFAlgorithm(List<ProcessData> processes)
        {
            ...
            while (completed < totalProcesses)
            {
                ...
                // Set first start time only once
                if (result.StartTime == -1)
                {
                    result.StartTime = currentTime;
                }

                // Run for 1 time unit because SRTF is preemptive
                remainingBurstTimes[currentProcess.ProcessID]--;
                currentTime++;

                // If finished, calculate final values
                if (remainingBurstTimes[currentProcess.ProcessID] == 0)
                {
                    completed++;
                    result.FinishTime = currentTime;
            ...
            results = processResults.Values.OrderBy(r => r.StartTime).ToList();
            return results;
        }
        ...
        private List<SchedulingResult> RunHRRNAlgorithm(List<ProcessData> processes)
        {
            ...
            while (completed.Count < processes.Count)
            {
                ...
                foreach (var process in availableProcesses)
                {
                    ...
                    double responseRatio = (waitingTime + process.BurstTime) / (double)process.BurstTime;
                ...
                currentTime = finishTime;
                completed.Add(selectedProcess.ProcessID);
            }
        ...
        private void DisplaySchedulingResults(List<SchedulingResult> results, string algorithmName)
        {
            ...
            var firstArrival = results.Min(r => r.ArrivalTime);
            var lastFinish = results.Max(r => r.FinishTime);
            var totalTime = lastFinish - firstArrival;

            var cpuUtilization = totalTime > 0 ? (totalBurstTime / totalTime) * 100.0 : 0;
            ...
            // CPU Utilization row
            var cpuItem = new ListViewItem("METRIC");
            cpuItem.SubItems.Add("CPU Utilization");
            ...
            // Throughput row
            var throughputItem = new ListViewItem("METRIC");
            throughputItem.SubItems.Add("Throughput");
            throughputItem.SubItems.Add($"{throughput:F2} proc/unit");
            ...
            // Response Time row
            var responseItem = new ListViewItem("METRIC");
            responseItem.SubItems.Add("Avg Response Time");
            responseItem.SubItems.Add($"{avgResponseTime:F2}");
            responseItem.SubItems.Add("");
            responseItem.SubItems.Add("");
            responseItem.SubItems.Add("");
            responseItem.SubItems.Add("");
            listView1.Items.Add(responseItem);
        ...
        private void btnSRTF_Click(object sender, EventArgs e)
        {
            ...
            if (processData.Count > 0)
            {
                ...
                DisplaySchedulingResults(results, "SRTF");
        ...
        private void ExportResultsButton_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("No results to export.", "Export Results",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            saveFileDialog.FileName = "SchedulingResults.csv";
                ...
                try
                {
                    using (StreamWriter writer = new StreamWriter(saveFileDialog.FileName))
                    {
                        ...
                        foreach (ListViewItem item in listView1.Items)
                        {
                            ...
                            {
                                string value = item.SubItems[i].Text.Replace(",", " ");
                                writer.Write(value);
